"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreatorUserActiveHistoryService = void 0;
const common_1 = require("@nestjs/common");
const constants_1 = require("../../../../Shared/domain/constants/constants");
const user_finder_service_1 = require("../../../../Shared/application/user/user-finder.service");
const UserActiveHistory_1 = require("../../domain/UserActiveHistory");
let CreatorUserActiveHistoryService = class CreatorUserActiveHistoryService {
    constructor(userActiveHistoryRepository, userFinderService) {
        this.userActiveHistoryRepository = userActiveHistoryRepository;
        this.userFinderService = userFinderService;
    }
    async run({ userId, startTime, endTime, }) {
        const user = await this.userFinderService.invoke(userId);
        if (!user)
            return;
        const userActiveHistory = UserActiveHistory_1.UserActiveHistory.create({
            userId: user.id.value,
            startTime: startTime,
            endTime: endTime,
        });
        await this.userActiveHistoryRepository.save(userActiveHistory);
    }
};
CreatorUserActiveHistoryService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(constants_1.USER_ACTIVE_HISTORY_REPOSITORY)),
    __metadata("design:paramtypes", [Object, user_finder_service_1.UserFinderService])
], CreatorUserActiveHistoryService);
exports.CreatorUserActiveHistoryService = CreatorUserActiveHistoryService;
//# sourceMappingURL=creator-user-active-history.service.js.map